...
const RecipeResultList = ({
  ...
  const [recipeList, setRecipeList] = useState<(Recipe & DbId)[]>();
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3100/recipe?" + useQueryFilter.toString(), {
      headers: {
  ...
  if (isLoading) {
    ...
    content = <>ERROR</>;
  } else if (typeof recipeList !== "undefined") {
    const prevButton = (
      <Button
        as={page === 1 ? undefined : Link}
        to={`/recipes/${tab}/${perPage}/${page - 1}`}
        isDisabled={page === 1}
    ...
    const nextButton = (
      <Button
        as={recipeList.length !== perPage ? undefined : Link}
        to={`/dishes/${tab}/${perPage}/${page + 1}`}
        isDisabled={recipeList.length !== perPage}
        rightIcon={
    ...
    if (recipeList.length === 0) {
      ...
      for (const recipe of recipeList) {
        dishDisplay.push(
          <WrapItem maxWidth={300} key={recipe.id}>
            <RecipeResult
              recipe={recipe}
              layout="vertical"
              withLink={true}
